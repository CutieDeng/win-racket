...
`racket9-9.2.3-windows-x86_64.zip` and `racket9-9.2.3-windows-x86_64-setup.exe` as GitHub Actions artifacts. It runs `nmake all` before the
...
the default cache path is inside the install directory. Release asset publishing is enabled. The workflow uploads `racket9-9.2.3-windows-x86_64.zip` and `racket9-9.2.3-windows-x86_64-setup.exe` to `CutieDeng/win-racket` release `v9.2.3` using the `GITHUB_TOKEN` repository secret.
